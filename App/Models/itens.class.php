  ...
  class Itens extends Connect
  {
    ...
    public function totalitens($value)
    {
      ...
      while ($row = mysqli_fetch_array($this->result)) {
        ...
        $itens = $this->listItens($idprodutos, $idFabricante);

        // montando cabecalho:

        if (!empty($itens)) {
          echo '<li>';
          echo '<b> Produto: ' . $itens['NomeProduto'];
          echo ' / Fabricante: ' . $itens['Fabricante'];
          echo '</b> Comprados: ' . $itens['QuantItens'];
          echo ' | Vendidos: ' . $itens['QuantItensVend'];
          echo ' | Em Estoque: ' . $itens['Estoque'];
          echo '</li>';
        } else {
          echo '<li>Não há itens para exibir</li>';
        }
      }
    }
  ...
  <div>
    ...
    public function editItens($value)
    {
      ...
      if ($row = mysqli_fetch_array($this->result)) {
        ...
        $dataCompra = new DateWorker($DataCompraItens, 'Y-d-m');
        $dataVenc   = new DateWorker($DataVenci_Itens, 'Y-d-m');

        return $resp = array('Itens' => array(
          'idItens' => $idItens,
          'QuantItens'   => $QuantItens,
          'ValCompItens' => $ValCompItens,
          'ValVendItens' => $ValVendItens,
          'DataCompraItens' => $dataCompra->toRealFormat(),
          'DataVenci_Itens' => $dataVenc->toRealFormat(),
